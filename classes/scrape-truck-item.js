      ...
      while (retries < maxRetries && !success) {
        try {
          await axios.get(url).then((response) => {
            ...
            let title = truck$(
              "#siteWrap > main > section > div.offer-header__row.hidden-xs.visible-tablet-up > h1"
            );
            ...
            let production_date = truck$(
              "div.offer-header__row.visible-xs > span > span:nth-child(1)"
            ); //get all element with nth-child for production date
            let mileage = truck$(
              "div.offer-header__row.visible-xs > span > span:nth-child(2)"
            ); //get all element with nth-child for production date
            let currency = truck$(
              "#siteWrap > main > section > div.offer-header__row.hidden-xs.visible-tablet-up > div.offer-price > div.wrapper > span > span"
            );
            currency = currency.text();
            let price = truck$(".offer-price").attr("data-price");
            let power = truck$(
              "#parameters > ul:nth-child(1) > li:nth-child(8)"
            ); //get all element with nth-child for power
